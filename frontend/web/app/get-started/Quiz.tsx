...
const Quiz: React.FC = () => {
  ...
  const addLeadingZero = (number: number): string => (number > 9 ? `${number}` : `0${number}`);
    ...
    <div className={styles.quizContainer}>
      {!showResult ? (
          ...
          <ul className={styles.choicesList}>
            {choices.map((choice, index) => (
              <li
                ...
                key={index}
                className={selectedAnswers[activeQuestion] === choice ? styles.selectedAnswer : ''}
              >
                {choice}
              </li>
            ))}
        ...
        <div className={styles.result}>
          ...
          <p>We appreciate your input and will use this information to tailor our services to better meet your needs.</p>
        </div>
      )}
    </div>
  );
